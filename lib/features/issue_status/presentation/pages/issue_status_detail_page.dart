  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return asyncValue.when(
      loading: () => Scaffold(
        ...
        appBar: AppBar(
          title: const Text('Issue Status Details'),
          elevation: 0,
                    ...
                    child: Column(
                      ...
                      children: [
                        Row(
                          ...
                          children: [
                            const Expanded(child: _Label('ISSUE STATUS')),
    ...
    return Column(
      children: [
        _HistoryExpansionTile(
          ...
          title: 'Created Info',
          subtitle:
              'Created by ${formatUser(status.createdUser, fallback: 'Unknown')}',
          initiallyExpanded: true,
          entries: [
            _HistoryEntry(
              label: 'Created By',
              value: formatUser(status.createdUser, fallback: 'Unknown'),
            ),
            _HistoryEntry(
              label: 'Created Date',
              value: formatDateSafe(status.createdDate),
            ),
          ],
        ),
        if (hasModified)
          _HistoryExpansionTile(
            ...
            title: 'Modified Info',
            subtitle:
                'Modified by ${formatUser(status.modifiedUser, fallback: 'Not modified')}',
        ...
        if (hasDeletedInfo)
          _HistoryExpansionTile(
            ...
            title: 'Deleted Info',
            subtitle:
                'Deleted by ${formatUser(status.deletedUser, fallback: 'Unknown')}',
            initiallyExpanded: true,
            entries: [
              _HistoryEntry(
                label: 'Deleted By',
                value: formatUser(status.deletedUser, fallback: 'Unknown'),
              ),
              _HistoryEntry(
                label: 'Deleted Date',
                value: formatDateSafe(status.deletedDate, fallback: 'N/A'),
              ),
            ],
          ),
      ],
